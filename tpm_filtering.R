...
# Constants
BASE <- "/scratch/mjpete11/limma_tutorial"

# Input
BRAIN_META <- file.path(BASE, "data/brain_metadata.csv")
TPM <- file.path(BASE, "data/GTEx_Analysis_2017-06-05_v8_RNASeQCv1.1.9_gene_reads.gct")

# Output
DF1 <- file.path(BASE, "data/filtered_by_sex.csv")
DF2 <- file.path(BASE, "data/region_tally.csv")
DF3 <- file.path(BASE, "data/filtered_by_region.csv")
DF4 <- file.path(BASE, "data/union_region_filtered.csv")

# Read in files
meta <- read.table(BRAIN_META, header = TRUE, sep = ",", stringsAsFactors = FALSE)
